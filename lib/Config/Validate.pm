...
my %types = ( integer => \&_validate_integer,
...
sub validate {
  ...
  _validate($new_config, $schema, []);
  return $new_config;
}

sub _validate {
  my ($cfg, $schema, $path) = @_;
  ...
  while (my ($canonical_name, $def) = each %$schema) {
    ...
    foreach my $name (@names) {
      ...
      if (lc($def->{type}) eq 'nested') {
        ...
        $types{$def->{type}}($cfg->{$canonical_name}, $def, \@curpath);
      }
...
sub _validate_hash {
  ...
  if (not defined $types{$def->{keytype}}) {
  ...
  while (my ($k, $v) = each %$value) {
    my @curpath = (@$path, $k);
    print "Validating ", _mkpath(@curpath), "\n" if $debug;
    $types{$def->{keytype}}($k, $def, \@curpath);
    if ($def->{child}) {
      _validate($v, $def->{child}, \@curpath);
    }
  }
}
